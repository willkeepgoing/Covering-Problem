...
def _obj_wrapper(args, kwargs, x):  # 多进程
    return area(x, *args, **kwargs)


def pso(l, u, small_num, swarmsize=40, maxiter=100, debug=False):
    time1 = time.time()
    args = ()  # 多进程
    kwargs = {}  # 多进程
    ...
    while it <= maxiter - 1:
        ...
        if fp[i_max] > fg:
            ...
            if debug:
                print(
                    'New best for swarm at iteration {:}: {:}    coverage: {:}'.format(it, cor_trans(small_num, g), fg))

        if debug:
            print('Best after iteration {:}: {:}    coverage: {:}'.format(it, cor_trans(small_num, g), fg))
