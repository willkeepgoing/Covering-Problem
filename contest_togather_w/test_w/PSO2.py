...
def pso2(l, u, small_num, swarmsize=40, maxiter=100, debug=False):
    ...
    while it <= maxiter - 1:
        ...
        if fp[i_max] > fg:
            ...
            if debug:
                print(
                    'New best for swarm at iteration {:}: {:}  目标区域像素点平均覆盖几率为: {:}%'.format(it, cor_trans(small_num, g), fg * 100 / big_area[0]))

        if debug:
            print('Best after iteration {:}: {:}  目标区域像素点平均覆盖几率为: {:}%'.format(it, cor_trans(small_num, g), fg * 100 / big_area[0]))
